"""
theme_manager.py - Theme management for the application
"""
...
class ThemeManager:
    ...
    def _setup_fonts(self):
        ...
        try:
            # Load military font
            font_id = QFontDatabase.addApplicationFont(config.MILITARY_FONT)
            if font_id < 0:
                print(f"Error: Could not load font {config.MILITARY_FONT}")
            else:
                font_family = QFontDatabase.applicationFontFamilies(font_id)[0]
                config.MILITARY_FONT_FAMILY = font_family
